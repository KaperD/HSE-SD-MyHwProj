...
func TestGetHomeworkPageStudentOk(t *testing.T) {
	...
	data, code, err := Get(server, "/student/homeworks/0")
	require.Nil(t, err)

	require.Equal(t, http.StatusOK, code)

	expectedHtmlData, err := ioutil.ReadFile("./test/resources/get_homework_student.html")
	require.Nil(t, err)
...
func TestGetHomeworksPageStudentOk(t *testing.T) {
	timeExample, err := time.Parse(time.RFC3339, "2022-05-01T17:32:28+03:00")
	require.Nil(t, err)
	initialHomeworks := []myhwproj.Homework{
		{
	...
	data, code, err := Get(server, "/student/homeworks")
	require.Nil(t, err)

	require.Equal(t, http.StatusOK, code)

	expectedHtmlData, err := ioutil.ReadFile("./test/resources/get_homeworks_student.html")
	require.Nil(t, err)
	expectedHtml := string(expectedHtmlData)
...
func TestGetSubmissionPageStudentOk(t *testing.T) {
	timeExample, err := time.Parse(time.RFC3339, "2022-05-01T17:32:28+03:00")
	require.Nil(t, err)
	initialHomeworks := []myhwproj.Homework{
		{
	...
	data, code, err := Get(server, "/student/submissions/1")
	require.Nil(t, err)

	require.Equal(t, http.StatusOK, code)

	expectedHtmlData, err := ioutil.ReadFile("./test/resources/get_submission_student.html")
	require.Nil(t, err)
	expectedHtml := string(expectedHtmlData)
